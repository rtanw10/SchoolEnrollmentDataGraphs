...
import model.GDPIndicator;
import model.Indicator;
import model.IndicatorType;
import model.LinkedList;
import model.SchoolEnrollmentIndicator;
...
{	
	...
	private static LinkedList<Country> createRandomListOfCountries(Country [] allCountries)
	{
		...
		// TODO: Define the Node class to receive a parameter which determines the data type. 
		//       This class will store the data and a reference to the next node.
		//       Define the LinkedList class to be a singly list list of Node objects.
	...
	/**
	 * Tests the contains() method of the LinkedList class. 
	...
	{		
		...
		// TODO: In class LinkedList update the contains() method so that it
		//       traverses the list and checks whether a Node exists whose
	 ...
	 */
	private static void addCountryToList(LinkedList list, int numOfYears, Indicator[] data){
		// Prompts the user for the number of elements they want to add to the list.
		System.out.println("Which country would you like to add to the list?");
		String countryToAdd = keyboard.nextLine();

		Country newCountry = new Country(countryToAdd);
		for (int i = 0; i < data.length; i++) {
			newCountry.setIndicatorForYear(data[i].getYear(), data[i]);
		}
		System.out.printf("Adding country with name %s to the end of the list.\n", newCountry.getName());
		list.add(newCountry);

		System.out.printf("Done with adding %s to the list.\n", newCountry.getName());
	}
	...
	public static void main(String[] args)
	{
		...
		for (int currentFileIndex = 0; currentFileIndex < filenames.length; currentFileIndex++)
		{
			...
			// If it has been seend, then we will get a reference to the previous
			...
			for (int countryIndex = 0; countryIndex < countries.length; countryIndex++)
			{
				...
				if (foundCountry == null)
				{
					// Create a Country object which has a LinkedList of Indicator objects
					// TODO: Update the constructor to take in only one argument:
					//       - a String for the name of the country
					// NOTE: Now that we are moving to a more flexible linked list of Indicator
					//       objects, the Country instance does not need to know the number of
					//       years which it needs to store Indicator objects ahead of time. 
					//       So, we remove the second argument from our previous implementation
					//       of the Country constructor.
				...
				for (int yearIndex = 0; yearIndex < dataForAllYears.length; yearIndex++)
				{
					...
					// If the current Country has been seen before, then we will get a reference to the
					// previous Indicator object stored and update it.
		...
		selection = printMenu();
		final String [] filenames2;

		// Determines the input file pathname.
		if (selection == 1 || selection == 3)
		{
			selectedIndicator = IndicatorType.GDP_PER_CAPITA;
			filenames2 = selection == 1 ? GDP_INPUT_SHORT : GDP_INPUT;
		}
		else if (selection == 2 || selection == 4)
		{
			// Note: alternatively we can set the selected indicator as SCHOOL_ENROLLMENT_SECONDARY
			selectedIndicator = IndicatorType.SCHOOL_ENROLLMENT_PRIMARY;
			filenames2 = selection == 2 ? ENROLLMENT_INPUT_SHORT : ENROLLMENT_INPUT;
		}
		else
		{
			selectedIndicator = IndicatorType.INVALID;
			filenames2 = INVALID_INPUT;
		}
		...
		for (int currentFileIndex = 0; currentFileIndex < filenames2.length; currentFileIndex++)
		{
			// Parse the CSV data file *based* on the input filename.
			CSVParser parser;
			try
			{
				parser = new CSVParser(filenames2[currentFileIndex]);
				// Given the childrenEnrolledInPrimary_short_12years.csv file, the output is:
				// 			School Enrollment In Primary (% net) updated at 7/25/18
				//
				// Given the gdpPerCapita_short_12years.csv file, the output is:
			...
			{
				System.err.printf("File name %s not found.", filenames2[currentFileIndex]);
				return;
			}
			catch (InvalidFileFormatException e)
			{
			...
			// For debugging purposes
			System.out.println("\nParsing filename " + filenames2[currentFileIndex]);
		...
		{
			selectedIndicator = IndicatorType.GDP_PER_CAPITA;
			filenames3 = selection == 1 ? GDP_INPUT_SHORT : GDP_INPUT;
		}
		else if (selection == 2 || selection == 4)
		{
			// Note: alternatively we can set the selected indicator as SCHOOL_ENROLLMENT_SECONDARY
			selectedIndicator = IndicatorType.SCHOOL_ENROLLMENT_PRIMARY;
			filenames3 = selection == 2 ? ENROLLMENT_INPUT_SHORT : ENROLLMENT_INPUT;
		}
		else
		{
			selectedIndicator = IndicatorType.INVALID;
			filenames3 = INVALID_INPUT;
		}
		...
		for (int currentFileIndex = 0; currentFileIndex < filenames3.length; currentFileIndex++)
		{
			// Parse the CSV data file *based* on the input filename.
			CSVParser parser;
			try
			{
				parser = new CSVParser(filenames3[currentFileIndex]);
				// Given the childrenEnrolledInPrimary_short_12years.csv file, the output is:
				// 			School Enrollment In Primary (% net) updated at 7/25/18
				//
				// Given the gdpPerCapita_short_12years.csv file, the output is:
			...
			{
				System.err.printf("File name %s not found.", filenames3[currentFileIndex]);
				return;
			}
			catch (InvalidFileFormatException e)
			{
			...
			// For debugging purposes
			System.out.println("\nParsing filename " + filenames3[currentFileIndex]);
		...
		System.out.println("Done with TestGenericList.");
